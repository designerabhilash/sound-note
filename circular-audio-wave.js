    ...
    nf.prototype = {
        ...
        dataToPoint: function(t, e, i) {
            var n = this.getAxis("x"),
                r = this.getAxis("y");
            return (i = i || [])[0] = n.toGlobalCoord(n.dataToCoord(t[0])), i[0] = r.toGlobalCoord(r.dataToCoord(t[0])), i
        },
...
class CircularAudioWave {
    constructor(t, e = {}) {
        ...
        if (this.defaultChartOption = {
            ...
            window.AudioContext ? (this.context = new AudioContext(), this.offlineContext = new OfflineAudioContext(2, 1323e3, 44100), 
            this.sourceNode = this.context.createBufferSource(), 
            this.offlineSource = this.offlineContext.createBufferSource(), 
            this.sourceNode.loop = !!this.opts.loop, this.analyser = this.context.createAnalyser()) : alert("Your browser does not support Web Audio API"), "sunburst" === this.opts.mode) {
    ...
    pause() {
        console.log(this.sourceNode.start())
        this.sourceNode && this.sourceNode.buffer ? 
        (this.playing = !1, this.presetOption(), this.sourceNode.stop(0), this._debouncedDraw()) 
        : 
        alert("Audio is not ready")
        
    }
    resume(index) {
        this.sourceNode && this.sourceNode.buffer ? (this.playing = !0, this.presetOption(), this.sourceNode.start(0, index/1000), this._debouncedDraw()) : alert("Audio is not ready")
    }
    destroy() {
        this.chart.dispose()
    }
    ...
    fixedDuration() {
        let audioDuration = this.sourceNode.buffer.duration;
        let calculatedDuration = audioDuration * 1000;
    ...
    _generateWaveData(t) {
        ...
        if ("sunburst" !== this.opts.mode) {
            for (let r = 1; r <= 360; r++) {
                var n = (t[r] - 0) * (this.maxChartValue - this.minChartValue) / 355 + this.minChartValue;
                n > i && (i = n), e.push([n, r])
                // if(t[r] === 0){
                // }
            }
    ...
    _getBPM(t) {
        ...
        let n = [];
        i.forEach((t, e) => {
            for (let r = 1; e + r < i.length && r < 2; r++) {
                let a = {
        ...
        document.getElementById("playit").innerText = "Engine Start";
